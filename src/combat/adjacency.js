...
//
// 타입마다 인접 규칙이 다르므로 배치가 곧 퍼즐이 된다:
//   드래곤 뭉칠수록 강해진다      → 한 덩어리로 몰아 놓기
//   고스트 혼자일수록 강해진다    → 일부러 떼어 놓기
//   강철   강철끼리 연결된다      → 줄로 잇기
//   격투/에스퍼 이웃을 강화한다   → 지원 배치
...
export const ADJ_RULES = {
  ...
  STEEL: {
    label: '자력 연결', kind: 'self',
    text: '인접한 강철 1기당 광역·관통 +12%',
    short: '강철끼리 연결',
  },
};

const PER = { DRAGON: 0.09, FIGHT: 0.07, PSYCHIC: 0.06, GHOST: 0.11, STEEL: 0.12 };
...
export function computeAdjacency(state) {
  ...
  for (const u of state.towers) {
    ...
    switch (u.def.field) {
      case 'DRAGON': {
        const n = near.length;
        if (n) { self.atk += per * n; self.notes.push(`${ADJ_RULES.DRAGON.label} ×${n}`); }
        break;
      }
      case 'GHOST': {
        ...
        if (empty) { self.atk += per * empty; self.notes.push(`${ADJ_RULES.GHOST.label} ×${empty}`); }
        break;
      }
      case 'STEEL': {
        const n = near.filter((x) => !isProp(x.def) && x.def.field === 'STEEL').length;
        if (n) { self.splash += per * n; self.notes.push(`${ADJ_RULES.STEEL.label} ×${n}`); }
        break;
      }
      case 'FIGHT':
      case 'PSYCHIC': {
        const stat = u.def.field === 'FIGHT' ? 'rate' : 'range';
        for (const n of near) {
