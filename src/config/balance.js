...
export const BALANCE = {
  ...
  // (아래 주석은 옛 설계 — 지금은 낼지 말지를 고르는 방식으로 바꿨다)
  //
  // 예전에는 자동으로 빠져나가고 못 내면 그냥 패배였다. "필요성이 없다"는
  // 피드백이 정확했다 — 낼 수 있으면 아무 일도 안 일어나고, 못 내면 판이 끝나서
  // 선택이 아니라 세금이었다.
  // 지금은 낼지 말지를 고른다. 불참하면 코인을 아끼는 대신 그 웨이브가 세진다.
  // 코인을 화력에 다 쏟는 길이 실제로 열려 있고, 그 대가가 눈에 보인다.
  rent: { everyN: 5, base: 190, growth: 1.46, skipHp: 0.30 },
  ...
  // 타입 상성 배율 (불꽃▶풀▶물▶불꽃)
  // 상성 배율은 원작 표(2배 / 0.5배)를 그대로 쓴다.
  // immune 은 원작의 무효(0배)를 대신하는 값 — 타워가 완전히 무력해지면
  // 고장으로 보이므로 "거의 안 통한다" 수준까지만 눌렀다.
  attrMult: { immune: 0.4 },
